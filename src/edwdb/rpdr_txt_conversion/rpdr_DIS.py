...
#%% Parse RPDR discharge summaries

data_root = os.path.normpath('/mnt/obi0/phi/ehr/rpdr/MAMMOGRAPHY2_20190905_142750')
save_dir = os.path.join(data_root, 'parquet','DIS2')
txt_filename = 'IH24_20190905_142750_Dis.txt'

def get_field_names(txt_file):
    df = pd.read_csv(txt_file, sep = '|', nrows = 10)
    return list(df.columns)

def parse_dis_report_list(report_list, field_names):
    """
...
for f, file in enumerate(file_list):
    ...
    # Convert to df
    df = parse_dis_report_list(report_list, field_names)

    # Add the folder name
    report_name = os.path.basename(os.path.dirname(os.path.dirname(file)))
    parquet_name = report_name+'_DIS.parquet'
