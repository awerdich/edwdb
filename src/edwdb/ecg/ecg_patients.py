...
meta_dir = os.path.join(parquet_dir, 'betablockerdata_parts')
...
def ecg_in_TestOrderInterval(idlist, ecg_df, med_df, medid_target_list):
        ...
        if mrn_med.shape[0] > 0:
            ...
            if mrn_med_target.shape[0] > 0:
                ...
                if ecg_med_int.shape[0] > 0:
                    ...
                    ecg_med_target_list.append(ecg_med_int)
            else:
                ...
                ecg_med_nontarget_list.append(ecg_med)
...
for chunk, idlist in enumerate(mrn_chunk_list):
    ...
    med_df = med_df.dropna(subset=['MGHMRN', 'OrderStartDTS', 'MedicationID']).\
        ...
        rename(columns={'MGHMRN': 'MRN', 'MedicationDSC': 'MedicationDSCDB'}).reset_index(drop=True)
    ...
    # We have two MedicationDSC columns. Drop the one from the DB
    target_df_ref = target_df.merge(ref_med, on='MedicationID', how='left').\
        drop(columns=['MedicationDSCDB']).drop_duplicates().reset_index(drop=True)
    nontarget_df_ref = nontarget_df.merge(ref_med, on='MedicationID', how='left').\
        drop(columns=['MedicationDSCDB']).drop_duplicates().reset_index(drop=True)
    ...
    target_df_ref = target_df_ref.astype({'OrderInterval': 'str'})

    # Save data
    target_df_ref.to_parquet(os.path.join(meta_dir, target_part_name))
    nontarget_df_ref.to_parquet(os.path.join(meta_dir, nontarget_part_name))
